...
### 4.6 自治体公式・公式観光カレンダー起点の季節イベント発見レーン

- **位置づけ**: 都道府県・市区町村の公式サイト、都道府県公式観光サイト、自治体または地域の観光協会が公開するイベント／観光カレンダーから、固定施設に紐づかない花火・祭り等を拾う**エリア起点の補助レーン**。§4.5 のプラットフォーム起点レーンとは別に並置し、置換しない。施設起点の `events-source-registry` tier、既存の掲載基準、source trust、HOLD 基準も変更しない。公式カレンダー掲載は候補発見の根拠であって採用の根拠ではない。
- **走査対象と深さ**: イベント公開済みの各都道府県について、(1) 都道府県公式のイベントカレンダー／市区町村イベント一覧、(2) 都道府県公式観光サイトのイベントカレンダー／季節特集、(3) **県内の全市区町村**の公式イベント・観光ページ／広報カレンダー、(4) 自治体が公式導線として案内する市区町村・広域観光協会のイベントページ、の順で確認する。都道府県や主要観光地の一覧だけで終えず、現行の都道府県公式市区町村一覧を母集団として全自治体を台帳化する。施設個別ページはこのレーンの母集団にせず、既存 registry 運用に残す。
- **市区町村カバレッジ台帳**: 1行を1市区町村とし、`prefecture`、`municipality`、`official_calendar_url`、`tourism_calendar_url`、`checked_at`、`season`、`candidate_count_before_dedupe`、`candidate_count_after_dedupe`、`zero_hit_recheck`、`notes` を残す。完了条件は台帳の市区町村集合が公式の現行一覧と一致すること。候補0件の市区町村は「イベントなし」とみなさず、サイト内検索とWeb検索を各1回追加し、前年同季の名称も検索して、結果または不採用理由を `zero_hit_recheck` に記録する。未走査自治体が1つでもあればその県の巡回は未完了とする。
- **季節先行スケジュール**: 春（3〜5月）・夏（6〜8月）・秋（9〜11月）・冬（12〜2月）ごとに、開始16週前に前年同季の名称と公式導線を棚卸しし、開始12週前に全市区町村の初回走査を行う。その後は開始8週前からシーズン終了まで、公開中都道府県を4バッチに分けて毎週1バッチ（各県は4週に1回）を差分走査する。候補0件、前年に採用候補があった自治体、HOLD中の候補は開始4週前と2週前にも再確認する。自治体告知が開催1〜3か月前に公開される一方、全国全市区町村を人手で確認する前提のため、12週前の全件走査と4週周期の再走査を組み合わせ、開催当日・直前だけの発見を防ぐ。
- **検索面と語彙**: 公式サイト内のカレンダー／検索（プラットフォーム内検索）とWeb検索の両方を使う。夏の基本語群は `(花火 OR 夏祭り OR 納涼 OR 盆踊り OR 灯籠流し OR 灯ろう流し OR 火祭り OR 大文字焼 OR 縁日 OR 山開き)` とし、`親子 OR 子ども OR キッズ OR ファミリー` も別検索する。Web検索の基本形は `site:{公式ドメイン} {対象年} ({季節語群})` と `{市区町村名} {対象年} ({季節語群}) 公式`。春は桜まつり・ひな祭り・こどもの日、秋は収穫祭・紅葉まつり・ハロウィン、冬は雪まつり・イルミネーション・冬花火・正月行事を加える。検索結果スニペットだけで候補情報を確定しない。
- **候補化と一次情報確認**: エリア一覧で surface した名称は候補台帳へ移し、対象年の主催自治体・主催者・会場による個別公式ページを開いて、開催日・時間・会場名と所在地・料金・予約方法・対象年齢／保護者同伴条件・開催状態・注意事項を確認する。`official_url`、`source_urls`、`source_checked_at`、`source_notes` は既存ルールどおり必須。自治体・観光協会の一覧だけ、前年情報だけ、検索結果スニペットだけでは掲載しない。固定施設との対応を一次情報で確定できないエリアイベントは `facility_id: null` と正式な `venue_name` を使い、このレーンの都合で施設レコードや registry tier を新設しない。
- **既存データとの突合ゲート**: 追加前に `data/events_data.json` と `data/summer_events_2026.json` の両方を、(1) id、(2) scheme／hostの大小、末尾 `/`、fragment、追跡queryを除いた正規化URL（`official_url` と `source_urls`）、(3) 正規化したタイトル＋会場＋開催日、の3キーで突合する。いずれかが一致すれば新規追加せず、同一イベントかを人手確認する。複数日・内包イベントは別イベントか同一祭りのプログラムかを一次情報で判断し、判断不能なら HOLD とする。
- **HOLD／不採用**: 必須情報不足、対象年不明、開催状態不明、中止・延期を否定できない、対象年齢／保護者条件不明、子ども向けでない、会場未確定、公式ページ取得不能は既存基準どおり HOLD または不採用とし、公式掲載を理由に緩和しない。開催済みイベントは本番データへ遡及追加せず、次季の名称監視リストへ確認日・公式URL・理由を残す。
- **記録と反映後ゲート**: 県ごとの走査 evidence を `.codex/municipal-calendar-lane-<prefecture>-<YYYY-MM-DD>.md` に残し、市区町村母集団、検索面、候補、公式URL、確認日、重複判定、HOLD／不採用理由を再現可能にする。採用反映時は `npm run events:validate`、`npm run events:regular:validate`、`npm run events:regular:test`、`npm run build` を通す。Summer Hub 専用ロジック、`feature_hubs`、DB schema はこの発見レーンでは変更しない。
