...
test("fixed 2026-07-21 canonical snapshot remains deterministic and clean", () => {
  ...
  assert.equal(first.diagnostics.mappableEventCount, 119);
  assert.equal(first.diagnostics.eventToFacilityEventCount, 94);
  ...
  assert.deepEqual(
    ...
    {
      inputEventCount: 475,
      mappableEventCount: 119,
      holdEventCount: 356,
      missingLocationCount: 0,
      excludedFacilityCount: 8,
      eventToFacilityEventCount: 94,
      eventToFacilityRecommendationCount: 427,
      facilityToEventFacilityCount: 1212,
      facilityToEventThreeCandidateCount: 308,
      facilityToEventRecommendationCount: 2030,
    },
  );
});
