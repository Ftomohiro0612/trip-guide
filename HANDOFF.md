...
**🆕 2026-06-28(最新): イベントUI改善2点+京都画像+方針転換(施設充実へ)**。
①**ヘッダー「🎪 イベント一覧」追加 + 施設ページ イベントカードをハブ`EventCard`再利用でリッチ化**(origin/main=`f42891d`・3ファイル[Header/EventCard/FacilityEvents]・`showFacilityLink`新設で施設ページの自己リンクのみ非表示・公式確認日/料金/年齢ラベル/#タグまで表示・ハブ非回帰・本番目視GREEN)。
②**京都100施設のWiki画像=本番LIVE完了**(origin/main=`4545d42`・fetch73→Codex除外18→**PM目視で追加除外2**[1604大正池グリーンパーク=ダム堤体/1661天橋立傘松公園=砂州景観で本人ズレ]→**採用53/null47**・/prefecture/kyoto画像53が200配信・1604/1661は404/参照0で画像なし表示・非画像フィールドdeep-equal不変・総数2028不変)。京都=施設100+画像53+イベント38。★PM目視=全55枚サムネ確認(sharp 220pxサムネ生成→Read)。
③**方針転換(2026-06-28・オーナー)=「見た目」より施設情報の充実+イベント取得を最優先**。画像展開はメインから降格(後追い/軽い並行)。**棚卸し結果**: 初期3県(静岡73/山梨71/長野74)がpeer band(122-145)から約50不足・イベントもほぼ0(静岡0/長野0/山梨1)。**進め方=静岡→山梨→長野のPhase2(各+50→~123)→registry同梱→イベント取得→Phase2追加分再巡回**。栃木/新潟/千葉/埼玉(施設十分・イベント薄)のイベント補完はトリオの後。正本計画=`.codex/shizuoka-yamanashi-nagano-phase2-plan-2026-06-28.md`。
④**静岡Phase2=実装中**(オーナーGO済・採用51件=`.codex/shizuoka-phase2-review.md`・候補母数150=`.codex/shizuoka-candidates-full-2026-06-28.md`・全35市町空白解消・屋内/知育/動物/水族館/工場見学厚め・海水浴は弓ヶ浜1件のみ・新規ID2039〜・**コード波及なし=データ+registry+sitemap**)。Codex本データ化中→PM裏取り(73→124/raw2079/visible2074/registry+51/既存deep-equal/audit0/URL住所座標未確定0)→push予定。

**🆕 2026-06-28: カテゴリアイコン全展開スイープ=本番LIVE完了(origin/main=`2f4d089`)**。共有`components/CategoryIcon.tsx`新規(画像あれば`categoryIconImages`のnext/image・無ければ絵文字フォールバック)+7ファイルで絵文字→透過webp画像化(トップ「カテゴリから探す」統一/カテゴリページ ヘッダー&関連チップ/施設詳細ヘッダー/施設カード プレースホルダ&県・カテゴリ行/県ページ カテゴリチップ/地図ポップアップ)。**OG画像2箇所(category/facility opengraph-image)は絵文字維持**。Codex lint/tsc/build PASS+Playwright(popup含む)→PM独立build green→sitemap restore(ノイズ差分除外)→クリーンFF push(`6cee63c..2f4d089`)→**本番目視GREEN**(トップ/施設/県/カテゴリ各ページ category webp配信・OG endpoint200で絵文字維持・commit=コード7ファイルのみでデータ/画像ファイル不変)。検索窓文言も本番反映済(`キーワードで検索（施設名・地域・カテゴリ・特徴など）`)。**▶次=京都100施設の画像展開(branch `kyoto-images`・STEP1 dry-run中)**。カテゴリページ地図下の絞り込み(県/特徴チップで地図+一覧を同時に絞る)は**京都画像の後に仕様化**予約(オーナー確認後GO)。
...
**🆕 /events/hiroshima イベント公開=本番LIVE完了(2026-06-27・origin/main=`30d78c4`・13県目のイベント対象県)**。registry巡回72件(seasonal32/monthly40・manual_hard3除外)→候補doc採用56→**PMトリム5(1919こども文化科学館のみ12→7・ハブ偏り回避)→採用51件/20施設**。events_data287→**338**(広島可視51)・events県メタ12→**13**・配線4点。**facilities不変・registryは1950道の駅福富のURLを`kohannosato.co.jp`へ1行修正(他不変・総数1580)**。保留5(1984宮島水族館[主力・開催日詳細取れず]含む)は今回見送り→後続小バッチ候補。PM独立裏取り全GREEN(候補56−トリム5=51突合・既存287deep-equal・facilities不変・registry1950のみ・tags20/終了済み0/id重複0・lint/tsc/build)→クリーンFF push→Codex本番Playwright+PM curl両GO(掲載中51・TZ罠なし・ハブ広島カード・施設節最大3件[1919=3/実数7]・既存12イベント県退行なし・console0)。**広島=施設75+イベント51で両用到達(17都府県目・中国地方初の両用県)**。
**🆕 広島(hiroshima)Phase2 施設拡張=本番LIVE完了(2026-06-27・origin/main=`bde5ab8`・+47→広島122)**。オーナー方針=愛知125/福岡125/兵庫123/大阪132のpeerに揃える(品質ゲート優先・115〜125着地可)。母数=stepB-review次回採用候補50+保留68→PM採用案CORE47(`.codex/hiroshima-phase2-review.md`)→Codex本データ化で**47件採用(新ID1992〜2038・予備補充0)**・広島75→**122**・total1981→**2028**・registry1580→**1627**。**コード波及なし**(県コードPhase1済=データ+registry+sitemapの3面)。**★PM裏取りの教訓(重要)**: 初回裏取りで監査stdoutパースが取りこぼし「新規high0/medium2」と誤報告→**構造化ファイル `.codex/facility_data_quality_issues.json` を正本にしたら実際は high5/medium5**(住所5=番地なし invalid_address・tag_category_conflict1・missing_experience1・thin_description3)。オーナーへ訂正→Codexが**全10件修正**(住所5=公式/自治体/公的観光サイトで番地レベル確認し補正[比治山公園1-1・呉市川尻町板休5502-37・御手洗65・西条本町17-1・三次町439-3]+座標整合・2013 rec tagsにexperience・4件descriptionに体験語)→**構造化ファイルで id1992-2038 high=0/medium=0**(low/info含め0)・全不変条件PASS→クリーンFF push。**強条件A〜G全件公式裏取り**(工場見学/見学ツアー/酒蔵ワイナリー[飲酒主目的にしない]/水辺ダム[安全注記]/季節[期間明記]/商業テナント[独立性]/同名混同[府中市≠府中町])。**本番検証GO**(PM curl+Codex LIVE Playwright PC1280/SP375): /prefecture/hiroshima122選・/map全体2023[=2028−exclude5]/広島122マーカー・新規施設詳細・既存県退行なし[fukuoka125/aichi125/osaka132/tokyo191]・既存イベント県退行なし[osaka43/aichi55/hyogo47/kyoto38/fukuoka37/gunma12/ibaraki31]・console/pageerror0・横スクロールなし。**★Codex NO-GOは誤検出**=/events表示337 vs events_data生338の比較ミス(東京`evt-219`サイバーセキュリティ・ミッションラリーが6/25終了の自然鮮度減・events_data生338は不変deep-equal)→可視/raw罠のイベント版`feedback_prefecture_visible_count_trap`。**広島=施設122+イベント51で施設バッチ一次完成(17都府県目)**。
**🆕 広島122件のWiki画像取得=本番LIVE完了(2026-06-27・origin/main経由)**。`scripts/fetch-wiki-images.ts`に**`--prefecture=<id>`フィルタを最小追加**(県単位で回せる・再利用可)→大阪等他県と分離して広島122だけ取得。フェッチ86→Codexが**要確認19件除外**(同名異所[逢瀬公園=福島/富山ファミリーパーク/鳴門運動公園=徳島]・自治体ページ・地図SVG・汎用・食品一般)→68採用→**PMが目視28件スポット確認し航空写真2件(1923ゲートパーク中央公園/1995比治山公園=衛星写真)を除外→最終66/122**。変更は広島66件のimage/image_attribution/image_sourceのみ(他施設/他フィールド/他県/events/registry/コード不変deep-equal)。attribution=Wikipediaページリンク(Commons作者/ライセンスとして過度に扱わない)。クリーンFF push→本番検証(採用66配信200・next/image表示・除外2件は画像参照0=壊れ画像なし・退行なし)。★画像の誤マッチ率≒0%(名前一致セーフガード+Codex除外+PM目視)・ヒット率54%は事前想定2〜4割超。**オーナー成功判定**。

**🆕 広島Phase2追加47施設(1992-2038)のイベント再巡回=本番LIVE完了(2026-06-27・origin/main=`1e5c7ca`・+13→広島イベント64)**。PMがregistryから巡回母数47件機械抽出(`_hiroshima_phase2_patrol_worklist.json`・weekly18/monthly22/seasonal7・除外0)→Codex読み取り巡回→候補doc(採用候補16/7施設・保留3・不採用37)→**PMレビューでハブ偏り検出**(ふくやま美術館2017が7件・イオンモール2000が4件=16中11が2施設に集中)→**オーナーGO=ふくやま7→4トリム**(オバケ?展[看板]+体験性強いファミリーWS3を残し小規模ミニWS/読み聞かせ3を落とす)→**最終13件/7施設**(2000:4/2017:4/他各1)。events_data338→**351**・events-onlyバッチ(facilities/registry/コード/sitemap不変・/events/hiroshimaはPhase1配線済)。PM裏取り全GREEN(既存338deep-equal・新13全hiroshima・タグ逸脱0・終了済み0・id重複0)→クリーンFF push→**Codex LIVE Playwright GO**(/events/hiroshima掲載中**64**・data-event-card64・施設節2000=3/2017=3[最大3件cap・ハブは4件]・既存イベント県維持・console/pageerror全ページ0・横スクロールなし)。★候補docのハブ偏りは1919前例(12→7)と同型でトリム判断。**広島=施設122+画像66+イベント64で17都府県目を完全クローズ達成**。

**🆕 県アイコン白枠修正=本番LIVE完了(2026-06-27・origin/main=`b09c23c`)**。トップ「エリアで探す」で京都/広島/群馬の3県だけ白い四角枠(`prefectureIconImages`→webpが透過になっていない=kyoto/hiroshima alpha無し・gunma alpha有りだが背景白塗り)。元PNG(.codex/prefecture_aicon/*.png)は全RGB白背景で、他14県は変換時に白→透過処理済みだがこの3県は未処理。Codexがsharpで**エッジ連結の白背景だけflood-fill透過**(内部白[daruma顔/鳥居淡色]は保持=穴あき防止)・256px・webp再生成→`public/images/prefectures/{kyoto,hiroshima,gunma}.webp`のみ置換。**PMがグレー背景合成プレビュー(.codex/icon-preview/)を目視**(白枠消失+穴あきなし確認)→他14県webp+コード/データ不変を裏取り→push→本番3県とも with alpha 新版に切替確認。

**▶次=画像の全国展開(県単位バッチ・オーナー確定方針)**: 広島で画像フロー成功→人気県を県単位で展開。**順番=大阪132→京都100→兵庫123→愛知125→福岡125**(オーナー確定 2026-06-28)。各県フロー=`fetch-wiki-images.ts --prefecture=<id>`取得→Codex除外(同名異所/自治体/地図SVG/汎用/食品)→**PM目視スポット確認で航空写真/地図的画像トリム**→採用画像のみpush→本番確認。取れない施設はnull維持・image系3フィールドのみ変更(他不変)。**大阪132が進行中**(branch`osaka-images`)。保留=広島1984宮島水族館イベント・愛知1776ジオスペース館。
...
- **/events/fukuoka イベント公開=本番LIVE完了(2026-06-26・origin/main=`6f479b7`・12県目のイベント対象県)**。福岡Phase1施設75の registry巡回対象67件(weekly14/biweekly3/monthly20/seasonal30・PMが`.codex/_fukuoka_events_patrol_worklist.json`へ機械抽出)→Codex読み取り巡回→候補doc`.codex/event-candidates-fukuoka-2026-06-26.md`(採用32/保留5/不採用49)→PMレビュー(件数3点突合一致32+**会場不一致の保留を確認**=1800うんちウォッチ[実会場が動植物園]/1831県庁会場茶道/SSL制約=Aichi1781の教訓どおりCodexが正しく除外)+**PMトリム3件**(1818響灘グリーンパークが7件突出→汎用枠[梅雨のワクワク工作会/動物お絵描き会/わくわくキッズランド]を質優先で落とし4件に・施設3件上限表示も考慮)→**オーナーGO「29件」**→Codex実装(ブランチ`fukuoka-events`)・events_data250→**279**(fukuoka可視29)+配線4点(EventPrefecture+fukuoka[`lib/events.ts`]/イベント県メタ/`app/events/page.tsx`ハブtitle・desc「…愛知・福岡」/next-sitemap+/events/fukuoka)+sitemap再生成。**facilities_data(1856)/registry(1455)不変**(events-onlyバッチ)。**PM独立データ裏取り全GREEN**(既存250件deep-equal削除0変更0・新29全てfukuoka・**除外3件未投入を機械確認**・タグ逸脱0/終了済み0/公式URL不備0/id重複0・facilities/registry差分0・lint/tsc/build PASS)→push(`389df10..6f479b7`クリーンFF)→**Codex本番Playwright+PM curl両GO**(/events/fukuoka掲載中**29**・TZ罠なし・ハブ福岡カード[掲載中29/登録75]・フィルタ動作[県278→29/今週末5/今月5/屋内23/無料5/予約不要9/好き1]・施設1818/1800/1820=ハブ各4件/施設ページ各3件[最大3件仕様]・既存osaka43/aichi55/hyogo47/kyoto38/gunma12/ibaraki31退行なし・/prefecture/fukuoka75・/map全体1851維持・console/pageerror0・横スクロールなし・PC1280/SP375)。dispatch=`.codex/_dispatch_fukuoka_events.txt`(巡回)/`_dispatch_fukuoka_events_impl.txt`(実装)/`_dispatch_fukuoka_events_prodverify.txt`(検証)。**福岡=施設75+イベント29で両用到達(16都府県目・九州初の施設+イベント両用県)**。**次=STEP9 Phase2拡張(~125)→STEP10追加分イベント再巡回で福岡完全クローズ(着手は次のオーナーGO待ち)**。

- **福岡Phase2施設拡張=本番LIVE完了(2026-06-27・origin/main=`283043a`・+50→福岡125)**。オーナーGO「+50→125」→PM採用案`.codex/fukuoka-phase2-review.md`(母数=台帳208のPhase1未採用~133から地域/テーマ均衡で50・福岡市6/北九州7/都市圏10/筑豊7/筑後10/京築町村10)→Codex本データ化(新規ID **1867〜1916**・total1856→**1906**・registry1455→**1505**[福岡125]・**コード波及なし**=データ+registry+sitemapのみ・新カテゴリ0・落とし0補充0)→**PM独立裏取りで既存施設1804チームラボ座標が11mナッジ(33.595→33.5951等)されていたのを捕捉**(Codexは「deep-equal」と誤申告→PM比較で1件modified検出→1804座標を元値へ復元`283043a`→既存id<=1866 modified0回復)→push(`b3e2ff7..283043a`クリーンFF)。海水浴場はPhase2でも不採用・水辺3件(遠賀川水辺館1890/昭和の森1883/猪野1886)に安全注記・独立5件(BOSS E・ZO/油山市民の森/平尾台自然観察C/響灘ビオトープ/石炭記念公園)採用・ひびき動物=吸収継続。PMデータ裏取り全GREEN(既存16都府県deep-equal modified0・new50 id1867-1916・registry1505・events279不変・コード差分0・新カテゴリ0・bbox外0・重複0・福岡audit high/medium0)。**Codex本番Playwright+PM curl両GO**(/prefecture/fukuoka125選/カード125/マーカー125・新規5施設200[1867/1873/1897/1899/1895]・/map福岡125/全体**1901**[raw1906−exclude5]・全16県フィルタ件数一致・/events/fukuoka29維持・既存16都府県+12イベント県退行なし[東京eventsは6/25終了で7→6=自然鮮度減]・console0・横スクロールなし・PC1280/SP375)。dispatch=`.codex/_dispatch_fukuoka_phase2.txt`(実装)/`_dispatch_fukuoka_phase2_prodverify.txt`(検証)。**★学び=PM独立裏取り>自己申告(Codexのdeep-equal誤申告を座標11mナッジで捕捉。co-location施設追加時は既存施設の座標が重複回避でナッジされていないか必ず確認)**。**福岡=施設125+イベント29で施設バッチ一次完成**。

- **STEP10 福岡Phase2追加50施設(facility_id 1867-1916)のイベント再巡回=本番LIVE完了(2026-06-27・origin/main=`a11c4a8`・commit「Add Fukuoka phase2 events」)**。PMが`events-source-registry.json`から巡回対象を機械抽出(`.codex/_fukuoka_phase2_patrol_worklist.json`・1867-1916の全50件=weekly7/monthly16/seasonal27・**not_suitable/no_event_source除外0=施設追加時にregistry行を同梱済で巡回対象がきれいに50件**)→Codex読み取り専用巡回→候補doc`.codex/event-candidates-fukuoka-phase2-2026-06-27.md`(採用9/7施設・保留0・不採用43・URL不達17)→PMレビュー(件数3点突合9=9=9をPM自前カウントで一致・統制20タグ逸脱0・終了済み0・会場一致確認・海川遊泳採用0=水系は園内芝生水遊び[1871]とガイド付き湿地観察[1875]のみ安全注記)+**PMトリム1件=facility-1881 奴国の丘ジュニアガイド養成講座**(春日市内在住の小4-6限定+5回連続の養成講座→どこからでも行ける親子おでかけイベントとして弱い・愛知の招待制/対象限定トリムと整合)→**オーナーGO「Option A=8件」**→Codex実装(ブランチ`fukuoka-phase2-events`・`data/events_data.json`単独・原子commit `a11c4a8`)・events_data279→**287**(fukuoka可視29→**37**)・**配線不要**(/events/fukuoka既存)。**PM独立機械照合(push前)全GREEN**(既存279件 deep-equal[削除0変更0・順序保存]・新8件すべてfukuoka・終了済み0・統制タグ逸脱0・公式URL非http0・id重複0・必須欠落0・facilities/registry/sitemap/コード差分0)→**PMがpush**(`089a701..a11c4a8`クリーンFF)→**Codex本番Playwright+PM curl両GO**(/events/fukuoka掲載中**37**・新規8件表示・トリム1881非表示・ハブ福岡カード「掲載中37/登録125」・施設ページのイベント節1867=2/1875=2/1871・1876・1879・1908=各1[最大3件仕様]・フィルタ[今週末5/今月5/屋内27/無料7/予約不要12/好き水遊び5]・既存osaka43/aichi55/hyogo47/kyoto38/gunma12/ibaraki31/tokyo6退行なし・/prefecture/fukuoka125・console/pageerror PC1280+SP375=0・横スクロールなし)。dispatch=`.codex/_dispatch_fukuoka_phase2_events_patrol.txt`(巡回)/`_dispatch_fukuoka_phase2_events_impl.txt`(実装)/`_dispatch_fukuoka_phase2_events_prodverify.txt`(検証)。**福岡=施設125+イベント37で完全クローズ(16都府県目・九州初の県を、施設もイベントもPhase2追加分の再巡回まで含めて閉じた)**。

**▶次セッション入口=新たな主題/県をオーナーと相談**。福岡まで含め全展開県(大阪132+43/兵庫123+47/京都100+38/愛知125+55/福岡125+37)がPhase2追加分の再巡回まで含めて完全クローズ。候補=(a)新県へ展開(北海道/沖縄は面積・季節・離島で重い大型県=後ろ・他の中規模県を先に)・(c)別主題=年間おでかけレポート無料ミニ版の仕様化(長期北極星`project_product_north_star`)。保留=愛知1776ジオスペース館(市広報PDF源)・1779竹島潮干狩りは施設公式で再裏取れたら後続小バッチ候補。**着手は次のオーナーGO待ち**。

**本番 HEAD = origin/main = `a11c4a8`**(福岡Phase2追加分イベント+8→fukuoka37・本番LIVE完了。直下=`283043a` 福岡Phase2施設+50→125/`6f479b7` 福岡イベント+29/`626df00` 福岡Phase1施設+75)。**全展開県が施設+イベント+Phase2再巡回まで完全クローズ: 大阪132+43/兵庫123+47/京都100+38/愛知125+55/福岡125+37**。**全国16都府県・イベント12県(東京/神奈川/山梨/千葉/埼玉/茨城/群馬/大阪/兵庫/京都/愛知/福岡)**。
