...
export function createRelatedFacilitySelector(facilities) {
  const recommendationEligibleFacilities = facilities.filter(
    (facility) =>
      facility.child_use_status !== "restricted" &&
      facility.child_use_status !== "not_allowed",
  );
  const facilitiesByPrefecture = groupBy(
    recommendationEligibleFacilities,
    ...
    const localCandidates = (
      ...
      .filter(
        (candidate) =>
          ...
          candidate.child_use_status !== "restricted" &&
          candidate.child_use_status !== "not_allowed",
      )
