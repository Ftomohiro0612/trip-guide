...
export default function DeleteVisitButton({
  ...
  return (
    <button
      ...
      className={
        variant === "subtle"
          ? "text-xs font-medium text-red-500 underline underline-offset-4 transition-colors hover:text-red-600 disabled:opacity-50"
          : "px-2.5 py-1.5 border border-red-200 text-red-500 rounded-lg text-xs font-medium hover:bg-red-50 transition-colors disabled:opacity-50"
      }
    >
      {loading ? "削除中…" : variant === "subtle" ? "記録を削除" : "削除"}
    </button>
  );
}
